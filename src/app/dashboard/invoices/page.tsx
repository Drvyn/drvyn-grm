...
import { 
  Plus, Search, Eye, X, Check, Trash2, Loader2, ChevronDown 
} from "lucide-react" 
...
function InvoicesPage() {
  ...
  // --- Logic to Fetch Job Card Data from API ---
  const jobCardIdParam = searchParams.get("jobCardId")
  // Fetch only if we have a parameter in the URL
  const { data: jobCardData, isLoading: isJobCardLoading } = useJobCard(jobCardIdParam || "")
  ...
  useEffect(() => {
    ...
    // If we have job card data from the API, populate the form
    if (jobCardData && jobCardIdParam) {
      
      // 1. Convert Parts and Services into Invoice Items
      const newItems = [
        ...(jobCardData.spareParts || []).map(p => ({
          description: p.name,
          quantity: p.quantity,
          unitPrice: p.price,
        })),
        ...(jobCardData.services || []).map(s => ({
      ...
      // 3. Pre-fill Form
      setFormData({
        jobCardId: jobCardData.id || jobCardData._id || jobCardData.booking_id,
        customer: jobCardData.customer,
        amount: totalAmount,
        items: newItems,
        date: new Date().toISOString().split("T")[0],
        dueDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
        status: "draft", // Default to draft so user can review
        notes: jobCardData.notes || "Thank you for your business!",
      })

      // 4. Open Form and Clear URL
      setEditingId(null)
      setShowForm(true)
      router.replace("/dashboard/invoices", undefined)
    }
  }, [user, jobCardData, jobCardIdParam, router])
  ...
  const handleSave = () => {
    if (!formData.customer || !formData.items.length) {
      alert("Please ensure customer name and items are filled.")
      return
    }
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="space-y-2">
                    ...
                    <Input value={formData.jobCardId} onChange={(e) => setFormData({ ...formData, jobCardId: e.target.value })} placeholder="e.g., JC001" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Customer Name *</label>
                    <Input value={formData.customer} onChange={(e) => setFormData({ ...formData, customer: e.target.value })} placeholder="Enter customer name" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={handleAddItem} variant="outline" size="sm"><Plus className="w-4 h-4 mr-1" /> Add Item</Button>
                  </div>
                    ...
                    {formData.items.map((item, idx) => (
                      <div key={idx} className="flex gap-2 items-end">
                        ...
                        <Input 
                          ...
                          className="w-24" 
                        />
                        <Button variant="ghost" size="sm" onClick={() => handleRemoveItem(idx)} className="hover:bg-destructive/10">
          ...
          <CardContent>
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <Input placeholder="Search by invoice number or customer..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <tbody>
                    {filteredInvoices.length > 0 ? (
                      filteredInvoices.map((invoice) => {
                        const invId = invoice.id || invoice._id || ""
                        return (
                          <tr key={invId} className="border-b border-border hover:bg-muted/50 transition-colors">
                            <td className="py-3 px-4 text-sm font-medium text-foreground">{invId.slice(-6).toUpperCase()}</td>
                            <td className="py-3 px-4 text-sm text-foreground">{invoice.customer}</td>
                            <td className="py-3 px-4 text-sm font-medium text-foreground">₹{invoice.amount.toFixed(2)}</td>
                            <td className="py-3 px-4 text-sm text-foreground">{invoice.date}</td>
                            <td className="py-3 px-4">
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <button onClick={(e) => e.stopPropagation()} className={`inline-flex items-center justify-between w-[100px] px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1 ${getStatusColor(invoice.status)}`}>
                                ...
                                <DropdownMenuContent align="start" className="w-[100px]">
                                  {["draft", "sent", "paid", "overdue"].map((st) => (
                                    <DropdownMenuItem key={st} onClick={(e) => { e.stopPropagation(); handleStatusChange(invId, st as Invoice["status"]); }} className="capitalize cursor-pointer text-xs py-2">
                                      {st}
                                    </DropdownMenuItem>
                                  ))}
                                </DropdownMenuContent>
                            ...
                            <td className="py-3 px-4 flex gap-2">
                              <Button variant="ghost" size="sm" onClick={() => handleEdit(invoice)} className="hover:bg-primary/10"><Eye className="w-4 h-4" /></Button>
                              <Button variant="ghost" size="sm" onClick={() => handleDelete(invId)} className="hover:bg-destructive/10"><Trash2 className="w-4 h-4 text-destructive" /></Button>
                            </td>
                          </tr>
                        )
                      })
