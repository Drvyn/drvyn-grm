...
import { 
  Wrench, Users, FileText, Settings, LogOut, Menu, ChevronsLeft, 
  Clock, BarChart3, Package, ShoppingCart, ListTodo, Layers, 
  Receipt, UserCog, Car 
} from "lucide-react"
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Grouped Navigation
  const navigationSections: NavSection[] = userRole === "admin" 
    ? [
        {
          title: "Overview",
          items: [
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className={cn(
        ...
        {(sidebarOpen || isMobile) && (
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="rounded-lg shadow-sm shrink-0 bg-white/80 p-1">
              <img src="/favicon3.png" alt="Logo" className="w-7 h-7" />
            </div>
        ...
        {!isMobile && (
          <button
            ...
            className="p-2 rounded-md transition-all duration-200 text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/80 focus:outline-none"
          >
        ...
        {navigationSections.map((section, idx) => (
          <div key={idx}>
            ...
            {(sidebarOpen || isMobile) && section.title && (
              <h4 className="px-3 mb-2 text-xs font-semibold text-muted-foreground tracking-wider">
                {section.title}
              </h4>
            )}
            ...
            <div className="space-y-1">
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={cn("w-5 h-5 flex-shrink-0 transition-colors", isActive ? "text-sidebar-primary-foreground" : "text-sidebar-foreground/60 group-hover:text-sidebar-foreground")} />
      ...
      <div className="p-3 shrink-0 border-t border-sidebar-border/50 space-y-1">
        <a
          ...
          className={cn(
            "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 group relative",
            pathname === "/dashboard/settings" ? "bg-sidebar-primary text-sidebar-primary-foreground font-medium shadow-sm" : "text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
            (!sidebarOpen && !isMobile) && "justify-center px-2"
          )}
        >
          <Settings className={cn("w-5 h-5 flex-shrink-0 transition-colors", pathname === "/dashboard/settings" ? "text-sidebar-primary-foreground" : "text-sidebar-foreground/60 group-hover:text-sidebar-foreground")} />
  ...
  return (
    <div className="flex h-screen bg-sidebar overflow-hidden">
      <aside className={cn("bg-sidebar transition-all duration-300 flex-col z-20 border-r border-sidebar-border/50 hidden md:flex overflow-x-hidden", sidebarOpen ? "w-64" : "w-20")}>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-sidebar shrink-0 flex items-center justify-between px-4 md:px-8 text-sidebar-foreground z-10 border-b border-sidebar-border/50 md:border-none">
          <div className="flex items-center gap-4 flex-1">
             <Button variant="ghost" size="icon" className="md:hidden text-sidebar-foreground/80" onClick={() => setMobileMenuOpen(true)}>
                <Menu className="w-5 h-5" />
              </Button>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-end">
               <span className="text-sm font-semibold text-sidebar-foreground capitalize hidden sm:inline-block">{userRole} Account</span>
               <span className="text-xs text-sidebar-foreground/60 hidden sm:inline-block">{typeof window !== 'undefined' ? localStorage.getItem("userEmail") : ""}</span>
            </div>
